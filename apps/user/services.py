...
def login():
    ...
    tokens = {
        ...
        'refresh_token': create_refresh_token(identity=username), 'userId': user.id
    }
...
@jwt_required
def edit():
    current_user = UserA.query.filter_by(username=get_jwt_identity()).first().serialize()
    if not current_user['role'] == 'Product Owner':
        return jsonify({"msg": current_user.usarname+" does not have privileges"}), 400
