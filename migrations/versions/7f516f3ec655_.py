"""empty message

Revision ID: 7f516f3ec655
Revises: 
Create Date: 2020-03-08 13:08:34.217365

"""
from alembic import op
import sqlalchemy as sa


# revision identifiers, used by Alembic.
revision = "7f516f3ec655"
down_revision = None
branch_labels = None
depends_on = None


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.create_table(
        "books",
        sa.Column("id", sa.Integer(), nullable=False),
        sa.Column("name", sa.String(), nullable=True),
        sa.Column("author", sa.String(), nullable=True),
        sa.Column("published", sa.String(), nullable=True),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_table(
        "userA",
        sa.Column("id", sa.Integer(), nullable=False),
        sa.Column("username", sa.String(length=100), nullable=False),
        sa.Column("first_name", sa.String(length=100), nullable=False),
        sa.Column("last_name", sa.String(length=100), nullable=False),
        sa.Column("role", sa.String(length=100), nullable=False),
        sa.Column("password", sa.String(length=100), nullable=False),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("username"),
    )
    op.create_table(
        "logger",
        sa.Column("id", sa.Integer(), nullable=False),
        sa.Column("user", sa.Integer(), nullable=True),
        sa.Column(
            "event",
            sa.Enum(
                "add_project",
                "add_sprint",
                "add_criteria",
                "add_test",
                "add_task",
                "update_project",
                "update_sprint",
                "update_criteria",
                "update_test",
                "activate_project",
                "pause_project",
                "delete_project",
                "search_project",
                "user_register",
                "user_login",
                "user_role_assign",
                name="loggerevents",
            ),
            nullable=False,
        ),
        sa.Column("date", sa.DateTime(), nullable=True),
        sa.Column("loged_module", sa.String(length=100), nullable=False),
        sa.ForeignKeyConstraint(["user"], ["userA.id"],),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_table(
        "projects",
        sa.Column("id", sa.Integer(), nullable=False),
        sa.Column("description", sa.Text(), nullable=True),
        sa.Column("user_id", sa.Integer(), nullable=True),
        sa.Column(
            "status",
            sa.Enum("completed", "paused", "active", name="projectstatus"),
            nullable=False,
        ),
        sa.Column("date_created", sa.DateTime(), nullable=True),
        sa.ForeignKeyConstraint(["user_id"], ["userA.id"],),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_table(
        "sprints",
        sa.Column("id", sa.Integer(), nullable=False),
        sa.Column("description", sa.Text(), nullable=True),
        sa.Column("user_id", sa.Integer(), nullable=True),
        sa.Column("project_id", sa.Integer(), nullable=False),
        sa.Column("date_created", sa.DateTime(), nullable=True),
        sa.Column("closed", sa.Boolean(), nullable=True),
        sa.ForeignKeyConstraint(["project_id"], ["projects.id"],),
        sa.ForeignKeyConstraint(["user_id"], ["userA.id"],),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_table(
        "stories",
        sa.Column("id", sa.Integer(), nullable=False),
        sa.Column("description", sa.Text(), nullable=True),
        sa.Column("project_id", sa.Integer(), nullable=True),
        sa.Column(
            "priority",
            sa.Enum("high", "medium", "low", name="storypriority"),
            nullable=False,
        ),
        sa.Column("epic", sa.Boolean(), nullable=True),
        sa.Column("done", sa.Boolean(), nullable=True),
        sa.Column("date_created", sa.DateTime(), nullable=True),
        sa.Column("sprint_id", sa.Integer(), nullable=True),
        sa.Column("parent_id", sa.Integer(), nullable=True),
        sa.ForeignKeyConstraint(["parent_id"], ["stories.id"],),
        sa.ForeignKeyConstraint(["project_id"], ["projects.id"],),
        sa.ForeignKeyConstraint(["sprint_id"], ["sprints.id"],),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_table(
        "acceptcriteria",
        sa.Column("id", sa.Integer(), nullable=False),
        sa.Column("description", sa.Text(), nullable=True),
        sa.Column("story_id", sa.Integer(), nullable=False),
        sa.Column("user_id", sa.Integer(), nullable=True),
        sa.Column("date_created", sa.DateTime(), nullable=True),
        sa.Column("approved", sa.Boolean(), nullable=True),
        sa.ForeignKeyConstraint(["story_id"], ["stories.id"],),
        sa.ForeignKeyConstraint(["user_id"], ["userA.id"],),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_table(
        "accepttests",
        sa.Column("id", sa.Integer(), nullable=False),
        sa.Column("description", sa.Text(), nullable=True),
        sa.Column("story_id", sa.Integer(), nullable=False),
        sa.Column("user_id", sa.Integer(), nullable=True),
        sa.Column("date_created", sa.DateTime(), nullable=True),
        sa.Column("approved", sa.Boolean(), nullable=True),
        sa.ForeignKeyConstraint(["story_id"], ["stories.id"],),
        sa.ForeignKeyConstraint(["user_id"], ["userA.id"],),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_table(
        "tasks",
        sa.Column("id", sa.Integer(), nullable=False),
        sa.Column("description", sa.Text(), nullable=True),
        sa.Column("story_id", sa.Integer(), nullable=False),
        sa.Column("sprint_id", sa.Integer(), nullable=False),
        sa.Column(
            "task_type",
            sa.Enum("develop", "design", "fix", "refact", name="tasktype"),
            nullable=False,
        ),
        sa.Column(
            "task_status",
            sa.Enum("new", "init", "to_test", "ended", name="taskstatus"),
            nullable=False,
        ),
        sa.Column(
            "task_class",
            sa.Enum("easy", "middle", "hard", name="taskclass"),
            nullable=False,
        ),
        sa.Column("init_date", sa.DateTime(), nullable=True),
        sa.Column("end_date", sa.DateTime(), nullable=True),
        sa.Column("duration", sa.Integer(), nullable=True),
        sa.Column("est_time", sa.Integer(), nullable=True),
        sa.Column("user_id", sa.Integer(), nullable=True),
        sa.ForeignKeyConstraint(["sprint_id"], ["sprints.id"],),
        sa.ForeignKeyConstraint(["story_id"], ["stories.id"],),
        sa.ForeignKeyConstraint(["user_id"], ["userA.id"],),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_table(
        "assign",
        sa.Column("user_id", sa.Integer(), nullable=True),
        sa.Column("task_id", sa.Integer(), nullable=True),
        sa.ForeignKeyConstraint(["task_id"], ["tasks.id"],),
        sa.ForeignKeyConstraint(["user_id"], ["userA.id"],),
    )
    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_table("assign")
    op.drop_table("tasks")
    op.drop_table("accepttests")
    op.drop_table("acceptcriteria")
    op.drop_table("stories")
    op.drop_table("sprints")
    op.drop_table("projects")
    op.drop_table("logger")
    op.drop_table("userA")
    op.drop_table("books")
    # ### end Alembic commands ###
