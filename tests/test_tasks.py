...
def test_get_taskBySprint(client, init_database):
    ...
    task0 = add_temp_task('test description', sprint0[0], TaskType.design,TaskStatus.init,TaskClass.hard, 1)
    task1 = add_temp_task('test description2', sprint0[0], TaskType.design,TaskStatus.init,TaskClass.hard, 1)
    ...
    assert (task0[0] in [elem['id'] for elem in tasks]) and (task0[1] in [elem['id'] for elem in tasks])
    assert len(tasks) == 2
